...
fn try_closing_text_group(tokenized: &mut Tokenized, grapheme: &str) {
    if tokenized
        .open_tokens
        .remove(&TokenKind::TextGroupOpen)
        .is_some()
    {
        if let Some(last) = tokenized.tokens.last() {
            tokenized.cur_pos.column += last.length();
        ...
        match last.kind {
            TokenKind::BoldClose => {
                if let Some(index) = tokenized.open_tokens.remove(&TokenKind::BoldOpen) {
                    ...
                    open_index = tokenized
                        .open_tokens
                        .remove(&TokenKind::BoldItalicOpen)
                        .expect("Closing token requires open token");
                    let open_token = tokenized
                        .tokens
                        .get_mut(open_index)
                        .expect("Got token index from hashmap");
                    ...
                    let new_pos = Position {
                        line: open_token.position.line,
                        column: open_token.position.column + open_token.length(),
                    };
                    // +1 because the inner token gets closed first
                    tokenized.tokens.insert(
                        open_index + 1,
                        Token {
                            kind: TokenKind::BoldOpen,
                            content: TokenKind::BoldOpen.as_str().to_string(),
                            position: new_pos,
                        },
                    );
                }
                ...
                } else if let Some(index) = tokenized.open_tokens.remove(&TokenKind::BoldItalicOpen)
                {
                    ...
                    tokenized.tokens.insert(
                        ...
                        Token {
                            ...
                            content: TokenKind::ItalicOpen.as_str().to_string(),
                            position: new_pos,
                        },
                    );
                } else {
                    ...
                    if next_token_is_space_or_newline {
                        ...
                        tokenized
                            .open_tokens
                            .insert(last.kind, tokenized.tokens.len());
                    }
                    tokenized.tokens.push(last);
                    return;
                }
            ...
            TokenKind::BoldItalicClose => {
                if let Some(index) = tokenized.open_tokens.remove(&TokenKind::BoldItalicOpen) {
                    ...
                    let bold_index = tokenized
                        .open_tokens
                        .remove(&TokenKind::BoldOpen)
                        .expect("Bold open must exist for bold-italic closing");
                    let italic_index = tokenized
                        .open_tokens
                        .remove(&TokenKind::ItalicOpen)
                        .expect("Italic open must exist for bold-italic closing");
        ...
        Some(mut last) => {
            if last.kind == TokenKind::ItalicOpen {
                ...
                if tokenized.open_tokens.get(&TokenKind::BoldOpen).is_some() {
                    let preceding_token = tokenized
                        .tokens
                        .last()
                        .expect("Tokens must not be empty, because open token exists");
                ...
                if (tokenized.open_tokens.contains_key(&TokenKind::ItalicOpen)
                    ...
                    || tokenized
                        .open_tokens
                        .contains_key(&TokenKind::BoldItalicOpen)
                {
                    tokenized.tokens.push(last);

                    let combined_close_token = Token {
                        kind: TokenKind::BoldItalicClose,
                        content: TokenKind::BoldItalicClose.as_str().to_string(),
                        position: tokenized.cur_pos,
                    };
                    ...
                    let combined_open_token = Token {
                        kind: TokenKind::BoldItalicOpen,
                        content: TokenKind::BoldItalicOpen.as_str().to_string(),
                        position: tokenized.cur_pos,
                    };
                    ...
                    .contains_key(&TokenKind::BoldItalicOpen)
                {
                    ...
                    match tokenized.open_tokens.get(&TokenKind::ItalicOpen) {
                        Some(italic_index) => {
                            let bold_index =
                                tokenized.open_tokens.get(&TokenKind::BoldOpen).unwrap();
