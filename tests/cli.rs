...
use clap::Parser;
use shlex::Shlex;
use unimarkup_rs::{config::{Config, OutputFormat, UmBlockElements}, um_error::UmError};
...
fn no_arguments_given() {
  ...
  assert_eq!(cfg.unwrap().um_file, PathBuf::new(), "UmFile set without being set over cli");
}
...
fn out_file_option_set() -> Result<(), UmError> {
  ...
  assert_eq!(cfg.out_file.unwrap(), PathBuf::from(out_file), "Output file not set correctly");
...
fn insert_path_option_set() -> Result<(), UmError> {
  ...
  assert_eq!(cfg.insert_paths.unwrap()[0], PathBuf::from(insert_path), "Insert path not set correctly");
...
fn dot_unimarkup_option_set() -> Result<(), UmError> {
  ...
  assert_eq!(cfg.dot_unimarkup.unwrap(), PathBuf::from(dot_unimarkup), "Dot-Unimarkup path not set correctly");
...
fn theme_option_set() -> Result<(), UmError> {
  ...
  assert_eq!(cfg.theme.unwrap(), PathBuf::from(theme), "Theme file not set correctly");
...
fn enable_elements_option_set() -> Result<(), UmError> {
  let um_filename = "file.um";
  let elements = vec![UmBlockElements::VerbatimBlock, UmBlockElements::DefinitionList];

  let options = format!("--enable-elements={},{}", elements[0].to_string(), elements[1].to_string());
  let args = get_args(&options, um_filename);

  let cfg: Config = Config::parse_from(args);
  let cfg_elements = cfg.enable_elements.unwrap();

  assert_eq!(cfg_elements[0], elements[0], "Enabled element 1 not set correctly");
  assert_eq!(cfg_elements[1], elements[1], "Enabled element 2 not set correctly");
...
fn disable_elements_option_set() -> Result<(), UmError> {
  ...
  let options = format!("--disable-elements={},{}", elements[0].to_string(), elements[1].to_string());
  let args = get_args(&options, um_filename);

  let cfg: Config = Config::parse_from(args);
  let cfg_elements = cfg.disable_elements.unwrap();

  assert_eq!(cfg_elements[0], elements[0], "Disabled element 1 not set correctly");
  assert_eq!(cfg_elements[1], elements[1], "Disabled element 2 not set correctly");
...
fn references_set_without_required_options() {
  ...
  assert_eq!(cfg.unwrap().references.unwrap()[0], PathBuf::new(), "References set without required options");
}
...
fn citation_style_set_without_required_options() {
  ...
  assert_eq!(cfg.unwrap().citation_style.unwrap(), PathBuf::new(), "Citation style set without required options");
}
...
fn reference_options_set() -> Result<(), UmError> {
  ...
  assert_eq!(cfg.citation_style.unwrap(), PathBuf::from(csl), "Citation style file not set correctly");
  assert_eq!(cfg.references.unwrap()[0], PathBuf::from(refs), "References file not set correctly");
...
fn fonts_option_set() -> Result<(), UmError> {
  ...
  assert_eq!(cfg.fonts.unwrap()[0], PathBuf::from(font), "Font file not set correctly");
...
fn overwrite_out_files_option_set() -> Result<(), UmError> {
  ...
  assert!(cfg.overwrite_out_files, "Overwrite-out-files not set correctly");
...
fn replace_preamble_set_without_required_options() {
  ...
  assert!(!cfg.unwrap().replace_preamble, "Replace preamble set without required options");
}
...
fn relative_insert_prefix_option_set() -> Result<(), UmError> {
  ...
  assert_eq!(cfg.relative_insert_prefix.unwrap(), PathBuf::from(insert_prefix), "Relative insert prefix not set correctly");
...
fn html_template_option_set() -> Result<(), UmError> {
  ...
  assert_eq!(cfg.html_template.unwrap(), PathBuf::from(template), "Html template not set correctly");
...
fn html_mathmode_option_set() -> Result<(), UmError> {
  let um_filename = "file.um";
  let mathmode = unimarkup_rs::config::HtmlMathmode::Embed;
  ...
  assert_eq!(cfg.html_mathmode.unwrap(), mathmode, "Html mathmode not set correctly");
